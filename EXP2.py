...
def O_index_visualizor(Cell_val, highest_layer, lowest_layer, array):
    ...
    if O_checker_result == 1:
        ...
        linear_mean = 0.9377
        linear_std = 0.0715
        if linear_result >= linear_mean - linear_std:
            return 1
        quadratic_result = quadratic_r_squared(y_values)
        quadratic_mean = 0.9149
        quadratic_std = 0.1109
        if quadratic_result >= quadratic_mean - quadratic_std:
            return 1
        return 0 
    ...
    for i in range(100):
        ...
        if os.path.exists(file_path):
            ...
            for cell_val, cell_info in cell_dict.items():
                cnt += 1
                passed += O_index_visualizor(cell_val, cell_info.highest_layer, cell_info.lowest_layer, array)
